...
(function () {
    ...
    panel.id = 'podcast-extractor-panel';

    // Get saved position or use defaults
    const savedTop = GM_getValue('panelTop', '20');
    const savedRight = GM_getValue('panelRight', '20');
    ...
    panel.innerHTML = `
        <div id="panel-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:10px; cursor:move; user-select:none; padding:5px; margin:-5px -5px 10px -5px; border-radius:4px;">
            <h3 style="margin:0; color:#333; font-size:15px; pointer-events:none;">&#127908; Podcast Extractor</h3>
            <button id="minimize-btn" style="background:none; border:none; cursor:pointer; font-size:16px; color:#999; pointer-events:auto;" title="Minimize">&#8212;</button>
        </div>
    ...
    function dragStart(e) {
        // Only drag if clicking on the header, not the minimize button
    ...
    function dragEnd() {
        if (isDragging) {
            ...
            const currentRight = window.innerWidth - parseInt(panel.style.left) - panel.offsetWidth;

            GM_setValue('panelTop', currentTop.toString());
            GM_setValue('panelRight', currentRight.toString());
        }
    }

    // Function to inject the panel
    function injectPanel() {
        if (!document.getElementById('podcast-extractor-panel')) {
            ...
            if (header) {
                ...
                header.addEventListener('mouseenter', () => {
                    if (!isDragging) header.style.background = '#f0f0f0';
                });
                header.addEventListener('mouseleave', () => {
                    if (!isDragging) header.style.background = '';
                });
            }
        }
    }
    ...
    const observer = new MutationObserver(() => {
        if (!document.getElementById('podcast-extractor-panel')) {
            console.log('Panel removed, re-injecting...');
            injectPanel();
        }
    });
    ...
    // Minimize toggle (runs after panel exists)
    setTimeout(() => {
        let minimized = false;
